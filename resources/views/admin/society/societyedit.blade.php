...
@section ('content')
    <div class="col-md-12">
        <form class="forms-sample" action="{{ route('societyedit',$area->society_id) }}" method="post"
              enctype="multipart/form-data">
            @csrf
            <div class="card">
                <div class="card-header card-header-primary">
                    <h4 class="card-title float-left">Update Area</h4>
                    <div class="btn-group float-right">
                        <a href="{{ route('societylist') }}" class="btn btn-warning"><i class="fa fa-arrow-left"></i>
                            Back </a>
                    </div>
                </div>
                                ...
                                <select class="form-control form-control-sm" id="city_id " name="city">
                                    @foreach($cities as $city)
                                        <option value="{{$city->city_id}}"
                                                data-cityName="{{ strtolower($city->city_name) }}" {{ $area->city_id==$city->city_id?'selected':'' }}>
                                            {{$city->city_name}}
                                        </option>
                                    @endforeach
                                </select>
    ...
    <script src="https://maps.googleapis.com/maps/api/js?key={{ $map }}&libraries=places&callback=initMap"
            async defer></script>
    <script>
        var city = '{{ strtolower($cities[0]->city_name) }}';
